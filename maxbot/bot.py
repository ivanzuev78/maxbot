...
class Bot:
    ...
    async def _request(self, method: str, path: str, params=None, json=None, headers=None, raise_for_status=True):
        if params is None:
            params = {}
    ...
    async def upload_file(
        ...
        if not file_path and not file_bytes:
            raise ValueError("Нужно передать либо file_path, либо file_bytes")
        ...
        files = {"data": ('stub', buffer, mime_type)}
    ...
    async def send_file(
        ...
        payload = await self.upload_file(file_path, file_bytes, media_type)
